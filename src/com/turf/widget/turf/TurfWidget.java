...
public class TurfWidget extends AppWidgetProvider {
	...
	{
		if(DEBUG){Log.v(DEBUG_STRING, "onEnabled");}
	...
	{
		if(DEBUG){Log.v(DEBUG_STRING, "onUpdate");}
		super.onUpdate(context, appWidgetManager, appWidgetIds);
	}

	public void onReceive(Context context, Intent intent)
	{
		if(DEBUG){Log.v(DEBUG_STRING, "onReceive");}
		
		if(DEBUG){Log.v("onReceive", intent.getAction());}
		
		if(PREFS_UPDATE.equals(intent.getAction()) || WIDGET_UPDATE.equals(intent.getAction()))
		{
		...
		else if(TURF_LAUNCH.equals(intent.getAction()))
		{
			...
			{
				Toast.makeText(context, "To use this function. \nPlease install the game Turf", Toast.LENGTH_LONG).show();
			}
			else
			{
	...
	public void onDeleted(Context context, int[] appWidgetIds)
	{
		...
		if(DEBUG){Log.v(DEBUG_STRING, "onDeleted");}
	...
	{
		if(DEBUG){Log.v(DEBUG_STRING, "onDisable");}
